...
# 遍历数字的每一位
number = 123
for digit_str in str(number):   # 将number转为可迭代对象
    digit = int(digit_str)      # 读取一个位置的数字以后转为 int
    print(digit)
...
for i, j, k in zip(list1, list2, list3):
    print(i, j ,k)  # 按列表长度最短的来，可以方便统一文本长度

# -------------------- sort ---------------------
# 常见 error
list4 = list3.sort()    # 因为sort的返回值为None，所以无法在下面的代码中迭代list4
# for i in list4:       # TypeError: 'NoneType' object is not iterable
#     print(i)

# 给元组排序，虽然元组的值不能修改，但可以转为list修改好以后，再转回 tuple
t = (3, 4, 56, 2, 4, 65)
