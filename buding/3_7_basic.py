...
# 切片
# 索引越界会报错，切片越界不会报错
# 在不知道字符串长度的情况下，不需要写额外的逻辑去判断长度
# 可以直接给一个比较大的值作为切片去获得字符串
list1 = [1, 2, 3, 4, 5, 6]
...
f = "123456"
print(f[1:5:1])
print(f[-1:-3])     # 空，因为此时步长需要为负值，正值无法输出有效结果
print(f[-1:-3:-1])
...
# -------------------- 列表 list ---------------------
list1 = [12, 3, 4, 5, 6, "abc", 23.3, [123], ["abc1"]]  # 列表里可以放不同类型的值
...
d = {"a" : 0, "b" : 1, "c" : 2}
print(d["a"])
# 如果输出不存在的键的值，索引会报错，但使用 get函数 不会报错
